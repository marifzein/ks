...
export default function AdminDashboard() {
  ...
  const cards = [
    { icon: ClipboardCheck, label: 'Registrasi Menunggu', value: fmt(pending.length), to: '/admin/registrasi', tone: 'bg-gold/20 text-[#8a6d00]' },
    { icon: Users, label: 'Total Anggota', value: fmt(stats.members), to: '/admin/anggota', tone: 'bg-primary/10 text-primary' },
    { icon: Newspaper, label: 'Berita', value: news.length, to: '/admin/berita', tone: 'bg-blue-500/10 text-blue-600' },
    { icon: CalendarDays, label: 'Kegiatan', value: events.length, to: '/admin/kegiatan', tone: 'bg-emerald-500/10 text-emerald-600' },
  ]
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink/50">Kelola anggota, registrasi, dan konten organisasi · [DATA SIMULASI]</p>
      </div>

      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((c) => (
          <Link key={c.label} to={c.to} className="card-ik group p-6 transition-all hover:-translate-y-0.5 hover:shadow-lift">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card-ik p-7">
          <div className="flex items-center justify-between">
            ...
            <Link to="/admin/registrasi" className="text-xs font-bold text-primary hover:underline">Semua →</Link>
          </div>
          <div className="mt-5 flex flex-col gap-3">
            {pending.slice(0, 4).map((r) => (
              <Link key={r.id} to="/admin/registrasi" className="flex items-center gap-4 rounded-2xl bg-surface p-4 transition hover:bg-surface-deep">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs font-semibold text-ink/45">{r.nomor} · {r.cabang}</p>
                </div>
                <Badge tone="gold">Menunggu</Badge>
              </Link>
            ))}
            {pending.length === 0 && <p className="p-6 text-center text-sm font-bold text-ink/40">Tidak ada registrasi menunggu.</p>}
          </div>
        </div>
